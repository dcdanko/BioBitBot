...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        self.taxa_hierarchy = ['phylum','class','order','family','genus','species']
        ...
        with openMaybeZip(treeF['fn']) as tF:
            ...
            for line in tF:
                ...
                for i, taxon in enumerate(hierarchy):
                    rParent = rParent.addNameIfNotPresent(taxon)
    ...
    def populateTreeSeqCounts(self):
        for countF in self.find_log_files( config.sp['metagenomics']['raw_count']['taxa']):
            with openMaybeZip(countF) as cF:
                cF.readline()
                sample = getSampleFromFilename(countF)
                for line in cF:
                    name, count = line.split()
                    count = int(count)
                    self.phylo_tree[name].seqCountsBySamples[sample] = count
    ...
    def buildAlignmentStatsChart(self):
        ...
        for alignStatF in self.alignment_stat_files:
            sample, taxa, totalReads, alignedReads = getAlignmentStats(alignStatF)
        ...
        #     plotData[sample] = {}
        #     for hiTaxa, loTaxa in zip(self.taxa_hierarchy[:-1],self.taxa_hierarchy[1:]):
        #         hiAligned = alignStats[sample][1][hiTaxa]
        #         loAligned = alignStats[sample][1][loTaxa]
        #         notLoAligned = max(0, hiAligned - loAligned) # We want the proportion of reads that ONLY aligned at the higher taxonomic rank
        #         plotData[sample][hiTaxa] = notLoAligned
        #     plotData[sample][self.taxa_hierarchy[-1]] = alignStats[sample][1][self.taxa_hierarchy[-1]]
        #     if 'gene' in stats:
        #         plotData[sample]['gene'] = stats['gene']

        #     totalAligned = sum(plotData[sample].values())
        #     nUnaligned = totalReads - totalAligned
        #     plotData[sample]['unassigned'] = nUnaligned
...
class TreeNode(object):
    ...
    def findSeqCountsThatDidNotAlignToChildren(self):
        childcount = {sample:0 for sample in seqCountsBySamples.keys()}
        for child in self:
            for sample, qty in child.seqCountsBySamples.items():
                childcount[sample] += qty

        self.topAlignedSeqCountBySample = {sample:qty for sample,qty in seqCountsBySamples.items()}
...
class Tree(object):
    ...
    def rPopulate(self,parent):
        for child in parent:
            self.all_nodes[child.name] = child
            if not child.isleaf():
                self.rPopulate(child) 
